-- Add the official DeepSeek valley/peak pricing channel for Anthropic groups
-- that were introduced after the original OpenAI-only fallback migration.

CREATE TEMP TABLE startwork_deepseek_anthropic_target_groups ON COMMIT DROP AS
SELECT
    g.id AS group_id,
    'anthropic' AS pricing_platform
FROM groups g
WHERE g.deleted_at IS NULL
  AND (
      g.name LIKE 'deepseek-anthropic-ecosystem-%'
      OR g.name LIKE 'deepseek-anthropic-private-%'
  );
...
WITH rebuilt AS (
    ...
    FROM groups g
    JOIN startwork_deepseek_anthropic_target_groups target ON target.group_id = g.id
...
SELECT
    ...
    'Startwork managed DeepSeek Anthropic group using Asia/Shanghai valley prices and 2x peak periods',
    'active'
FROM startwork_deepseek_anthropic_target_groups target
WHERE NOT EXISTS (
    SELECT 1
...
CREATE TEMP TABLE startwork_deepseek_anthropic_target_channels ON COMMIT DROP AS
...
WITH rewritten AS (
    ...
    JOIN startwork_deepseek_anthropic_target_channels target
...
FROM startwork_deepseek_anthropic_target_channels target
CROSS JOIN (
    VALUES
        (
...
WHERE NOT EXISTS (
    ...
    WHERE existing.channel_id = target.channel_id
      AND LOWER(BTRIM(existing.platform)) = target.pricing_platform
      AND existing.models = pricing.models
);
