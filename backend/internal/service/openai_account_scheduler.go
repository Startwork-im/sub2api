...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if shouldClearStickySession(account, req.RequestedModel) || !isOpenAIPlatformAccount(account, req.AllowedPlatforms) || !account.IsSchedulable() {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, nil
	}
	...
	account = s.service.recheckSelectedOpenAIAccountFromDB(ctx, account, req.RequestedModel, req.RequireCompact, req.AllowedPlatforms)
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	accounts, err := s.service.listSchedulableAccounts(ctx, req.GroupID, req.AllowedPlatforms)
	if err != nil {
	...
	for i := range accounts {
		...
		if !account.IsSchedulable() || !isOpenAIPlatformAccount(account, req.AllowedPlatforms) {
			continue
		}
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		fresh := s.service.resolveFreshSchedulableOpenAIAccount(ctx, candidate.account, req.RequestedModel, false, req.AllowedPlatforms)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(fresh, req) {
			continue
		}
		fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, fresh, req.RequestedModel, false, req.AllowedPlatforms)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(fresh, req) {
			continue
		}
...
func (s *OpenAIGatewayService) SelectAccountWithScheduler(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, []string{PlatformOpenAI}, requiredTransport, "", requireCompact)
}

func (s *OpenAIGatewayService) SelectOpenAIChatAccountWithScheduler(
	ctx context.Context,
	groupID *int64,
	previousResponseID string,
	sessionHash string,
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, []string{PlatformOpenAIChat}, requiredTransport, "", requireCompact)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, []string{PlatformOpenAI}, OpenAIUpstreamTransportHTTPSSE, requiredCapability, false)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, []string{PlatformOpenAI}, OpenAIUpstreamTransportHTTPSSE, OpenAIImagesCapabilityBasic, false)
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				selection, err := s.selectAccountWithLoadAwareness(ctx, groupID, sessionHash, requestedModel, effectiveExcludedIDs, requireCompact, allowedPlatforms)
				if err != nil {
					return nil, decision, err
				}
				if selection == nil || selection.Account == nil {
